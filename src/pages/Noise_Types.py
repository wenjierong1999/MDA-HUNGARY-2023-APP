...
grouped_noise_weekday = df_noiseTypes_bar.groupby(['weekday', 'noise_event_laeq_primary_detected_class']).size().reset_index(name='count')
...
df_humidity_gt_80_week = df_humidity_gt_80.groupby(['weekday', 'noise_event_laeq_primary_detected_class']).size().reset_index(name='count_gt_80_week')
# Group the data by weekday and noise type for Humidity>80 = 0
df_humidity_lt_80_week = df_humidity_lt_80.groupby(['weekday', 'noise_event_laeq_primary_detected_class']).size().reset_index(name='count_lt_80_week')
...
def update_bar_chart_week(btn4,btn5,btn6):
    ...
    fig_week.update_layout(
        ...
        xaxis_title='Weekdays. 0 is Monday; 6 is Sunday',
        yaxis_title='Types Count',
        barmode='stack'  # Set the bar mode to stack for different color segments
    )
